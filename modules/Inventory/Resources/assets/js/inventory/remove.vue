            ...
            <div class="form-body">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div
                            class="form-group"
                            :class="{ 'has-danger': errors.quantity_remove }"
                        >
                            ...
                            <small
                                ...
                                v-if="errors.quantity_remove"
                                v-text="errors.quantity_remove[0]"
                            ></small>
                        </div>
                    </div>
                    <div
...
export default {
    ...
    methods: {
        ...
        clickSelectLotsGroup() {
            if (!this.form.quantity_remove)
                return this.$message.error("Ingrese la cantidad a retirar.");

            if (isNaN(this.form.quantity_remove))
                return this.$message.error(
                    "La cantidad a retirar no es un número válido."
                );
            ...
            if (this.form.lots_enabled) {
                if (!this.form.selected_lots_group)
                    return this.getObjectResponse(
                        false,
                        "Debe seleccionar los lotes."
                    );

                if (
                    this.getTotalCompromiseQuantity() !=
                    parseFloat(this.form.quantity_remove)
                )
                    return this.getObjectResponse(
                        false,
                        "La cantidad a retirar es diferente del total comprometido."
                    );
            }
        ...
        async submit() {
            if (this.form.series_enabled) {
                ...
                if (
                    this.form.lots.length !==
                    parseInt(this.form.quantity_remove)
                ) {
                    return this.$message.error(
            ...
            if (!validet_lots_group.success)
                return this.$message.error(validet_lots_group.message);

            this.loading_submit = true;
            await this.$http
                .post(`/${this.resource}/remove`, this.form)
                .then(response => {

    ...
    public function getKardexReportCollection(&$balance, $selected_warehouse_id = null)
    {
        ...
        switch ($this->inventory_kardexable_type) {
            ...
            case $models[0]: //venta DOCUMENT
                ...
                // Parseo para notas de creditos a través de api con facturas externas
                $cpe_doc_asoc = isset($inventory_kardexable->note) ? (
                    isset($inventory_kardexable->note->affected_document) ?
                        $inventory_kardexable->note->affected_document->getNumberFullAttribute()
                        : $inventory_kardexable->note->getNumberFullAttributeExternal()
                ): '-';

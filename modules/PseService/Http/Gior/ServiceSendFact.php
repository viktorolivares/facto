...
final class ServiceSendFact
{
    ...
    public function sendXml($xmlUnsigned, $filename)
    {
        ...
        try {
            ...
            $body = [
                'file_name' => $filename,
                'file_content' => base64_encode($xmlUnsigned)
            ];
    ...
    public function sendXmlSigned($filename, $xmlSigned, $hasSummary = false)
    {
        ...
        try {
            ...
            if ($th->hasResponse()) {
                ...
                $description = $entity_validator . implode(', ', $error['errors'] ?? []);
                // $parser_error = $this->parserCodeErrors($error['errors'][0]);
                Log::info('PSE SEND XML SIGNED ERROR', [
                    'error' => $error,
                ]);
                return [
                    'success' => false,
    ...
    public function getEndpointsByProvider($providerId, $soapTypeId)
    {
        ...
        return [
            ...
            'generate' => $soapTypeId == 2 ? Endpoints::SENDF_GENERATE : Endpoints::SENDF_BETA_GENERATE,
    ...
    public function validationCodeResponseIntegration($document_status, $message)
    {
        ...
        switch ($code) {
            ...
            case 5:
                $state = '01';
                Log::error("PSE ERROR: Code {$document_status}; Description: {$message}");
                break;
            default:
                $state = '05';
                break;

...
class DispatcherController extends Controller
{
    ...
    public function searchDispatchers(Request $request)
    {
        $input = $request->input('input', null);
        return Dispatcher::query()
            ->where('is_active', true)
            ->generalWhereLikeColumn('number', $input)
            ->generalOrWhereLikeColumn('name', $input)
            ->get()
            ->transform(function ($row) {
                return [
                    'id' => $row->id,
    ...
    public function toggleActive(Request $request, $id)
    {
        ...
        $dispatcher->is_active = $request->is_active;
        $dispatcher->save();

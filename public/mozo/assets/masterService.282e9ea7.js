...
import { u as useUserSession, a as useCompanySession, p as provideApi } from "./index.8c6daf4a.js";
import { d as defineStore, u as useStorage, r as ref, a as computed } from "./vendor.dca42141.js";
const useProductSession = defineStore("productSession", () => {
  ...
  function updateProductsStock(stockData) {
    stockData.forEach((stockItem) => {
      const product = products.value.find((p) => p.id === stockItem.item_id);
      if (product) {
        product.synced_stock = parseFloat(stockItem.available);
      }
    });
  }
  return {
...
class useSocket {
  ...
  sendCompany() {
    const data = {
      ruc: companySession$1.company.number,
      user: userSession$1.email
    };
    socket == null ? void 0 : socket.emit("data-company", data);
  }
  openTable(id) {
    const data = {
      table_id: id
    };
    socket == null ? void 0 : socket.emit("open-table", data);
  }
  resetTable(id) {
    const data = {
      table_id: id
    };
    socket == null ? void 0 : socket.emit("reset-table", data);
  }
  updateTable(table) {
    const data = {
      table
    };
    socket == null ? void 0 : socket.emit("update-table", data);
  }
  updateTableGroups() {
    socket == null ? void 0 : socket.emit("table-groups-updated");
  }
  startStockSync() {
    socket == null ? void 0 : socket.emit("start-stock-sync");
  }
  stopStockSync() {
    socket == null ? void 0 : socket.emit("stop-stock-sync");
  }
}
socket == null ? void 0 : socket.on("updateUserList", (users) => {
  console.log("Users list", users);
});
socket == null ? void 0 : socket.on("connect", () => {
  console.log("socket conectado");
});
socket == null ? void 0 : socket.on("reset-table-envs", () => {
  MesaService.syncMesasAndEnvironments();
});
socket == null ? void 0 : socket.on("set-open-table", (table_id) => {
  MesaService.syncMesa(table_id);
});
socket == null ? void 0 : socket.on("set-reset-table", (id) => {
  MesaService.syncMesa(id);
});
socket == null ? void 0 : socket.on("update-user-role", (data) => {
  const user_email = data.user_email;
  if (userSession$1.email == user_email) {
    const role_code = data.role_code;
    userSession$1.setRole(role_code);
  }
});
socket == null ? void 0 : socket.on("update-stock", (stockData) => {
  const productSession2 = useProductSession();
  if (stockData && stockData.data) {
    productSession2.updateProductsStock(stockData.data);
  }
});
const use = new useSocket();
const useMesaSession = defineStore("mesaSession", () => {
  ...
  async function setShippedMesa(id) {
    ...
    if (index !== -1) {
      ...
      await MesaService.saveMesa(mesas.value[index]);
      use.updateTable(mesas.value[index]);
    }
  }
  ...
  async function openMesa(mesa) {
    ...
    if (index >= 0) {
      ...
      mesas.value[index].is_venta_por_consumo = false;
      await MesaService.saveMesa(mesas.value[index]);
      use.openTable(mesas.value[index].id);
    }
  }
  async function calculaTotalBag() {
    ...
    use.updateTable(mesaSelectedRef.value);
  }
...
const saveMesa = async (payload) => {
  try {
    ...
    use.updateTable(data.data || payload);
    return data;
  } catch (err) {
...
export { COMMAND_BAR as C, DEFAULT_ENVIRONMENT as D, MasterService as M, STATUS_NEW as S, useMesaSession as a, COMMAND_KITCHEN as b, MesaService as c, MESA_NO_DISPONIBLE as d, MESA_DISPONIBLE as e, MESA_SHAPES as f, useProductSession as g, MESA_SHAPES_SQUARE as h, MESA_SHAPES_CIRCLE as i, masterService as m, use as u };

...
<table class="full-width mt-10 mb-10">
    <thead class="">
    <tr>
        <th width="8%" class="border-top-bottom desc-9 text-left">CANT</th>
        <th width="57%" class="border-top-bottom desc-9 text-left">DESCRIPCIÓN</th>
        <th width="17%" class="border-top-bottom desc-9 text-right">P.UNIT</th>
        <th width="18%" class="border-top-bottom desc-9 text-right">TOTAL</th>
    </tr>
    </thead>
    <tbody>
    @php
    ...
    @if($hay_fusionados)
    <tr>
        <td class="text-center desc-9 align-top font-bold">{{ number_format($cantidad_fusionada, 0) }}</td>
    ...
    @if($document->total_exonerated > 0)
        <tr>
            <td colspan="3" class="text-right font-bold desc">OP.
    ...
    @if($document->subtotal > 0)
        ...
        @endphp
        <tr>
            <td colspan="3" class="text-right font-bold desc">{{ $labelSubtotal }}: {{ $document->currency_type->symbol }}</td>
    ...
    @if($document->total_discount_with_igv > 0)
        <tr>
            <td colspan="3" class="text-right font-bold desc">DESCUENTO
    ...
    @if($document->total_charge > 0)
        @if($document->charges)
            ...
            @endphp
            <tr>
                <td colspan="3" class="text-right font-bold desc">CARGOS ({{$total_factor}}
                    %): {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_charge, 2) }}</td>
            </tr>
        @else
            <tr>
                <td colspan="3" class="text-right font-bold desc">CARGOS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_charge, 2) }}</td>
            </tr>
        @endif
    @endif
    ...
    @if ($document->retention)
        <tr>
            <td colspan="3" class="text-right font-bold desc">IMPORTE TOTAL: {{ $document->currency_type->symbol }}</td>
            <td class="text-right font-bold desc">{{ number_format($document->total, 2) }}</td>
        </tr>
        <tr>
            <td colspan="3" class="text-right font-bold desc">IMPORTE NETO: {{ $document->currency_type->symbol }}</td>
    ...
    @if(($document->retention || $document->detraction) && $document->total_pending_payment > 0)
        <tr>
            <td colspan="3" class="text-right font-bold desc">M. PENDIENTE: {{ $document->currency_type->symbol }}</td>

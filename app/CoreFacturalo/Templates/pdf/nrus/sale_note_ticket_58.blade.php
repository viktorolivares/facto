...
<table class="full-width mt-10 mb-10">
    <thead class="">
    <tr>
        <th width="12%" class="border-top-bottom desc-9 text-left">CANT</th>
        <th width="52%" class="border-top-bottom desc-9 text-left">DESCRIP.</th>
        <th width="18%" class="border-top-bottom desc-9 text-right">P.UNIT</th>
    ...
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-left desc-9 align-top">
                ...
                @php
                    $meta_parts = [];
                    if(!empty($row->item->internal_id)) $meta_parts[] = 'Cód: '.$row->item->internal_id;
                    if(!empty($row->item->unit_type_id)) $meta_parts[] = 'Und: '.$row->item->unit_type_id;
                @endphp
                @if(count($meta_parts))
                    <br/><span style="font-weight: normal; font-size: 8px; color:#8a8a8a;">{!! implode(' &middot; ', $meta_parts) !!}</span>
                @endif
    ...
    @endforeach
        ...
        {{-- @if($document->total_taxed > 0)
            <tr>
                <td colspan="3" class="text-right font-bold desc">OP. GRAVADAS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_taxed, 2) }}</td>
            </tr>
        @endif --}}
         @if($document->total_discount_with_igv > 0)
            <tr>
                <td colspan="3" class="text-right font-bold desc">{{(($document->total_prepayment > 0) ? 'ANTICIPO':'DESCUENTO TOTAL')}}: {{ $document->currency_type->symbol }}</td>
        ...
        <tr>
            <td colspan="1" class="text-left font-bold desc" style="white-space: nowrap;">Productos: {{ rtrim(rtrim(number_format(collect($document->items)->sum(function ($item) { return (float) data_get($item, 'quantity', 0); }), 2, '.', ''), '0'), '.') }}</td>

...
<table class="full-width mt-10 mb-10">
    ...
    <tbody>
    <tr>
        <td width="12%" class="ticket-58 border-top-bottom desc text-left">CANT</td>
        <td width="50%" class="ticket-58 border-top-bottom desc text-left">DESCRIP.</td>
        <td width="19%" class="ticket-58 border-top-bottom desc text-right" style="padding-right:5px;">P.UNIT</td>
        <td width="19%" class="ticket-58 border-top-bottom desc text-right">TOTAL</td>
    </tr>
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-left desc align-top text-uppercase">
                ...
                @php
                    $meta_parts = [];
                    if($row->item->internal_id) $meta_parts[] = 'Cód: '.$row->item->internal_id;
                    if($row->item->unit_type_id) $meta_parts[] = 'Und: '.$row->item->unit_type_id;
                @endphp
                @if(count($meta_parts))
                    <br/><span style="font-weight: normal; font-size: 8px; color:#8a8a8a; text-transform: none;">{!! implode(' &middot; ', $meta_parts) !!}</span>
                @endif
        ...
        <tr>
            <td colspan="4" class="ticket-58 border-bottom"></td>
        </tr>
    @endforeach
    {{-- OP. EXPORTACIÓN oculto en NRUS --}}
    @if($document->total_free > 0)
        <tr>
            <td colspan="2" class="desc-ticket text-uppercase">OP. GRATUITAS:
    ...
    @if($document->total_unaffected > 0)
        <tr>
            <td colspan="2" class="desc-ticket text-uppercase">OP. INAFECTAS:
                {{ $document->currency_type->symbol }}</td>
            <td colspan="2"
    ...
    @if($document->total_exonerated > 0)
        <tr>
            <td colspan="2" class="desc-ticket text-uppercase">OP. EXONERADAS:
                {{ $document->currency_type->symbol }}</td>
            <td colspan="2"
    ...
    @if($document->total_plastic_bag_taxes > 0)
        <tr>
            <td colspan="2" class="desc-ticket text-uppercase">ICBPER:
                {{ $document->currency_type->symbol }}</td>
            <td colspan="2"
    ...
    @if($document->total_charge > 0)
        @if($document->charges)
            ...
            @endphp
            <tr>
                <td colspan="2" class="desc-ticket text-uppercase">CARGOS ({{$total_factor}}%):
                    {{ $document->currency_type->symbol }}</td>
                <td colspan="2"
        ...
        @else
            <tr>
                <td colspan="2" class="desc-ticket text-uppercase">CARGOS:
                    {{ $document->currency_type->symbol }}</td>
                <td colspan="2"
                    class="text-right desc-ticket text-uppercase">{{ number_format($document->total_charge, 2) }}</td>
            </tr>
    ...
    <tr>
        ...
        <td colspan="1" class="desc-ticket text-uppercase">TOTAL A PAGAR:
            {{ $document->currency_type->symbol }}</td>
    ...
    @if(($document->retention || $document->detraction) && $document->total_pending_payment > 0)
        <tr>
            <td colspan="2" class="desc-ticket text-uppercase">M. PENDIENTE:
                {{ $document->currency_type->symbol }}</td>
            <td colspan="2"
    ...
    @if($balance < 0)
        <tr>
            <td colspan="2" class="desc-ticket text-uppercase">
                VUELTO:
